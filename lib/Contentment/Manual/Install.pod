...
In either case (and certainly others), you may wish to customize the F<.htaccess> file. The important lines are:

  RewriteRule "^$"    "cgi-bin/handler.cgi/index.html" [L]
  RewriteRule "(.*)$" "cgi-bin/handler.cgi/$1"         [L]

If you're using a custom handler, then replace the C<handler.cgi> on both lines with the name of your handler (C<handler-path.cgi> if you name it as suggested in the previous section).

If you've relocated F<cgi-bin>, then you need to make sure the start of the line locates it. For example, if the URL to your Contentment site is:
...
You can just add a "C</>" to the front of each part starting with "C<cgi-bin>".
